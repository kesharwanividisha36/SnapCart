...
const LiveMap = dynamic(
  () => import("@/components/LiveMapClient"),
  { ssr: false }
);
...
// import LiveMap from './LiveMapClient'
        ...
        <div className='bg-white border rounded-xl shadow-xl p-6'>
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            <BarChart data={todayEarning}>
                  ...
                  onClick={()=>window.location.reload()}>Refresh Earnings</button>
        </div>
      </div>
    </div>
  )
...
if(activeOrder && userLocation){
  return (
    <div className='p-4 pt-[120px] min-h-screen bg-gray-50'>
      ...
      <p className='text-gray-600'>order#{activeOrder.order._id.slice(-6)}</p>
    ...
    <button 
    onClick={sendOtp}
      className='w-full py-4 bg-green-600 text-white text-center rounded-lg'>{sendOtpLoading?<Loader size={16} className='animate-spin text-white text-center'/>:"Mark as delivered"}</button>
       )}
       {
        showOtpBox && 
         <div className='mt-4'>
 <input type="text" className='w-full py-3 border rounded-lg text-center '
 placeholder='Enter OTP' maxLength={5} onChange={(e)=>setOtp(e.target.value)} value={otp} />
 <button className='w-full mt-4 bg-blue-600 text-white text-center py-3 rounded-lg 'onClick={verifyOtp} >{verifyOtpLoading?<Loader size={16} className='animate-spin text-white text-center'/>:"Verify OTP"}</button>
    ...
    <div className='w-full min-h-screen bg-gray-50 p-4'>
      <div className='max-w-3xl mx-auto'>
          ...
          {assignments.map((a,index)=>(
            <div key={index} className='p-5 white rounded-xl shadow mb-4 '>
