...
useEffect(():any=>{
  ...
  return (
    ...
    >
        <div className='flex flex-col justify-between md:flex-row items-start md:items-center gap-3 border-b border-gray-100 px-5 py-4 bg-linear-to-r from-green-50 to-white'>
<div>
    <h3 className='text-lg font-semibold text-gray-800'><span className='text-green-700 font-bold'>#{order?._id?.toString()?.slice(-6)}</span></h3>
...
{order.paymentMethod=="cod"? <div className='flex items-center gap-2 text-gray-700 text-sm'>
    ...
    Cash On Delivery
     ...
     {order.assignedDeliveryBoy && <><div className='mt-4 bg-blue-50 border border-blue-200
rounded-xl p-4 flex items-center justify-between '>
  ...
  <a href={`tel:${order.assignedDeliveryBoy.mobile}`} className='bg-blue-600 text-white text-xs px-3 py-1.5 rounded-lg hover:bg-blue-700 transition'>Call</a>
   ...
   <button className='w-full flex items-center justify-center gap-2 bg-green-600 
     text-white font-semibold px-4 py-2 rounded-xl shadow' onClick={()=>router.push(`/user/track-order/${order._id?.toString()}`)}><TruckIcon size={18}/>Track Your Order</button>
      </>
      }
...
<p className='text-xs text-gray-800 font-semibold'>₹{Number(item.price)*item.quantity}</p>
            </div>
        ))}
    </div>
