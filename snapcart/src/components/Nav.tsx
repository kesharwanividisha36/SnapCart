...
import { Boxes, ClipboardCheck, Cross, LogOut, LogOutIcon, Menu, Package, Plus, PlusCircle, Search, ShoppingCartIcon, User, X } from 'lucide-react'
...
setSearchBarOpen(false)
    ...
    const sideBar=menuOpen?createPortal(
      <AnimatePresence>
       ...
       bg-linear-to-b form-green-800/90 via-green-700/80 to-green-900/90
       backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px rgba(0,255,100,0.3)] 
       ...
       >
        ...
        <div className='flex flex-col gap-3 font-medium mt-6'>
         <Link href={"/admin/add-grocery"} className='flex  gap-3 font-medium p-3 rounded-lg bg-white/10
         hover:bg-white/20 hover;pl-4 transition-all'>
            <PlusCircle className='w-5 h-5'/>Add Grocery</Link>
          <Link href={"/admin/view-grocery"} className='flex  gap-3 font-medium p-3 rounded-lg bg-white/10
         hover:bg-white/20 hover;pl-4 transition-all'><Boxes className='w-5 h-5'/>View Grocery</Link>
          <Link href={"/admin/manage-orders"} className='flex  gap-3 font-medium p-3 rounded-lg bg-white/10
         hover:bg-white/20 hover;pl-4 transition-all'><ClipboardCheck className='w-5 h-5'/>Manage Orders</Link>
        </div>
<div className='my-5 border-t border-white/20'></div>
<div className='flex items-center gap-3 text-red-300 font-semibold mt-auto hover:bg-red-500/20 p-3 rounded-lg transition-all'onClick={async()=>await signOut({callbackUrl:"/"})}><LogOut className='w-5 h-5 text-red-300'/>Log Out</div>
       </motion.div>
      </AnimatePresence>,
      document.body
    ):null
  ...
  return (
    ...
    rounded-2xl shadow-lg shadoe-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50 '>
    <Link href={"/"} className='text-white font-extrabold text-2xl sm:text-3xxl tracking-wide
...
<Search className='text-green-600 w-6 h-6'onClick={()=>setSearchBarOpen((prev)=>!prev)}/>
        ...
        <div className='hidden md:flex items-center gap-4'>
          ...
          font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all'>
            <PlusCircle className='w-5 h-5'/>Add Grocery</Link>
          <Link href={"/admin/view-grocery"} className='flex items-center gap-2 bg-white text-green-700 
          font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all'><Boxes className='w-5 h-5'/>View Grocery</Link>
          <Link href={"/admin/manage-orders"} className='flex items-center gap-2 bg-white text-green-700 
          font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all'><ClipboardCheck className='w-5 h-5'/>Manage Orders</Link>
        </div>
        <div className='md:hidden bg-white rounded-full w-11 h-11 flex flex-col items-center justify-center shadow-md overflow-hidden'onClick={()=>setMenuOpen(prev=>!prev)}>
          <Menu className='text-green-600 w-6 h-6 flex-col'/>
        </div>
        </>}
     
        <div className='relative' ref={profileDropDown}>
  <div className='bg-white rounded-full w-11 h-11 flex items-center justify-center overflow-hidden shadow-md
  hover:scale-105 transition-transform'onClick={()=>setOpen(prev=>!prev)}>
...
animate={{opacity:1,y:0,scale:1}}
  ...
  >
   ...
   {user.role=="user" &&  <Link href={"/user/my-orders"} className='flex items-center gap-2 px-3 py-3 hover:bg-green-50 rounded-lg text-gray-700 
