...
import { ArrowLeft, DivideSquareIcon, EyeIcon, EyeOff, Key, Leaf, Loader2, LockIcon, LogIn, Mail, UserIcon} from 'lucide-react'
...
function RegisterForm({previousStep}:propType) {
  ...
  return (
    ...
    px-6 py-10 bg-white relative'>
        <div className='absolute top-6 left-6 items-center flex gap-2 text-green-700
        hover:text-green-800 transition-color cursor-pointer'onClick={()=>previousStep(1)}>
        ...
        className='flex flex-col gap-5 w-full max-w-sm'>
            ...
            <div className='relative'>
               <UserIcon className='absolute left-3 top-3.5 w-5 h-5 text-grat-400'/>
                <input type="text" placeholder='Your Name' className='w-full border border-gray-300 rounded-xl
                py-3 pl-10 pr-4 text-gray-900  focus:ring-2 focus:ring-green-500 focus:outline-none'
                onChange={(e)=>setName(e.target.value)}
                value={name}/>
            </div>
            <div className='relative'>
               <Mail className='absolute left-3 top-3.5 w-5 h-5 text-grat-400'/>
            ...
            <div className='relative'>
               <LockIcon className='absolute left-3 top-3.5 w-5 h-5 text-grat-400'/>
                ...
                value={password}/>
                {
                    ...
                    onClick={()=>setShowPassword(false)}/>:<EyeIcon className='absolute right-3 top-3.5 w-5 h-5 text-gray-500 cursor-pointer'
            ...
            </div>
            {
              (()=>{
                const formValidation=name!=="" && email!=="" && password!=""
                    return <button disabled={!formValidation || loading} className={`w-full font-semibold py-3 rounded-xl transition-ll duration-200
