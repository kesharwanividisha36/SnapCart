...
import { decreaseQuantity, increaseQuantity, removerFromCart } from '@/redux/cartSlice'
...
function Cartpage() {
  ...
  return (
    <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative '>
      <Link href={"/"} className='absolute -top-2 left-0 flex items-center gp-2 text-green-700
...
>
  ...
  <p className='text-gray-600 textmb-6'>Your Cart is empty. Add some groceries to continue shopping!</p>
  ...
  <div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
    ...
    <AnimatePresence>
      {cartData.map((item,index)=>(
        ...
        hover:shadow-xl transition-all duration-300 border-gray-100'
        >
        <div className='relative w-28 h-28 sm:w-24 sm:h-24 md:h-28 flex-shrink-0
        ...
        onClick={()=>dispatch(removerFromCart(item._id))}><Trash2 size={18}/></button>
        </motion.div>
      ))}
    </AnimatePresence>
    </div>
      ...
      transition={{duration:0.3}}
      className='bg-white rounded-2xl shdow-xl p-6 h-fit sticky top-24 border border-gray-100 flex flex-col'
      >
        <h2 className='tex-lg sm:text-xl font-bold text-gray-800 mb-4'>Order Summary</h2>
        ...
        <motion.button whileTap={{scale:0.95}} className='w-full bg-green-600 text-white py-3 rounded-full
        hocer:bg-green-700 transition-all font-semibold text-sm sm:text-base'onClick={()=>router.push("/user/checkout")}>
          Proceed to CheckOut
        </motion.button>
      </motion.div>
    </div>
