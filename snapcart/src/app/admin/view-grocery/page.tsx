    ...
    useEffect(()=>{
        const getGroceries=async()=>{
            try {
                ...
                console.log("GROCERIES 👉", result.data)
            } catch (error) {
                console.log(error)
            }
        }
...
const handleSearch=(e:React.FormEvent)=>{
  ...
  return (
    <div className='pt-4 w-[95%] md:w-[85%] mx-auto pb-20'>
       ...
       hover:bg-green-200 text-green-700 font-semibold px-4 py-2 rounded-full
       ...
       initial={{opacity:0,y:10}}
        ...
        placeholder='Search By Name & Category....'value={search} onChange={(e)=>setSearch(e.target.value)}/>
       </motion.form>
...
{filtered?.map((g,i)=>(
        ...
        >
            ...
            <div className='flex-1 flex flex-col justify-between w-full'>
                <div>
                    ...
                    <p className='text-gray-500 capitalize text-sm'>{g.category}</p>
                </div>
                <div className='mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
                    <p className='text-green-700 font-bold text-lg'>
                        ₹{g.price}/<span className='text-gray-500 text-xm font-medium ml-1'>{g.unit}</span>
                    </p>
                    <button className='bg-green-600 text-white px-4 py-2 rounded-lg text font-semibold
                    flex items-center justify-center gap-2 hover:bg-green-700 transition-all'
    ...
    <motion.div
           ...
           className='bg-white rounded-xl shadow-xl w-[360px] p-6 relative '
>
    <div className='flex justify-between items-center mb-4'>
        <h2 className='text-2xl font-bold text-green-700'onClick={handleEdit}>Edit Grocery</h2>
       <button className='text-gray-600 hover:text-red-600' onClick={()=>setEditing(null)}><X size={18}/></button> 
    </div>
    <div className='relative h-50 w-full rounded-lg overflow-hidden mb-4 border border-gray-200 group'>
        {imagePreview && <Image
        src={imagePreview}
        alt={editing.name}
        fill
        ...
        group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity'><Upload size={28} className='text-green-500'/></label>
    ...
    <div className='flex justify-end gap-2 mt-4'>
        ...
        disabled={loading}
        >{loading?<Loader size={18}/>:"Edit Grocery"}</button>
        <button className='px-4 py-2 rounded-lg bg-red-600 text-white flex items-center gap-2 hover:bg-red-700 transition-all'
        onClick={handleDelete}  disabled={deleteLoading}>{deleteLoading?<Loader size={18}/>:"Delete Grocery"}</button>
    </div>
